...
make all
total_threads_list="1 2 4 6 8 16 24 36 48 60"
total_clients=4
repetitions=20
MAX_PACKS=1000000
num_port=1820
...
counters="r538288,r53a088,r53c488,r534188"
...
do
	...
	for ((i=1 ; $i<=$repetitions ; i++))
	{
		...
		{
			#Lanzar Cliente
			./clientTesis --intensive --packets $(($MAX_PACKS*$amplificador)) --ip 127.0.0.1 --port $num_port > /dev/null &
		}
...
do
	...
	for ((i=1 ; $i<=$repetitions ; i++))
	{
		...
		for (( k = 0; k < serverSockets; k++ ))
		{
			...
			{
				#Lanzar Cliente
				./clientTesis --intensive --packets $(($MAX_PACKS*$amplificador)) --ip 127.0.0.1 --port $(($k+$num_port)) > /dev/null &
			}
		}
