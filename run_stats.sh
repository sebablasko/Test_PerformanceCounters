...
# 		newFile="perfStat_UDPMultiThread"

# 		if (($serverThreads >= 0 & $serverThreads < 10)); then newFile=$newFile"_00"$serverThreads; fi
# 		if (($serverThreads >= 10 & $serverThreads < 100)); then newFile=$newFile"_0"$serverThreads; fi
# 		if (($serverThreads >= 100)); then newFile=$newFile"_"$serverThreads; fi

# 		if (($i >= 0 & $i < 10)); then newFile=$newFile"_00"$i".data"; fi
# 		if (($i >= 10 & $i < 100)); then newFile=$newFile"_0"$i".data"; fi
# 		if (($i >= 100)); then newFile=$newFile"_"$i".data"; fi
...
# 		newFile="perfStat_devNullMultiThread"
...
# 			newFile="perfStat_UDPMultiSocket"

# 			if (($serverSockets >= 0 & $serverSockets < 10)); then newFile=$newFile"_00"$serverSockets; fi
# 			if (($serverSockets >= 10 & $serverSockets < 100)); then newFile=$newFile"_0"$serverSockets; fi
# 			if (($serverSockets >= 100)); then newFile=$newFile"_"$serverSockets; fi

# 			if (($i >= 0 & $i < 10)); then newFile=$newFile"_00"$i; fi
# 			if (($i >= 10 & $i < 100)); then newFile=$newFile"_0"$i; fi
# 			if (($i >= 100)); then newFile=$newFile"_"$i; fi
...
echo ""
echo "Testing UDP SingleThread Multisocket with ReusePort Transmission"
for ((serverSockets=1 ; $serverSockets<=$limite_threads ; serverSockets=2*serverSockets))
{
	packages=$(($maxPackages/$serverSockets))
	echo $serverSockets" Sockets with reuseport, each socket receive "$packages" packages"
	...
	for ((i=1 ; $i<=$repetitions ; i++))		
	{
		...
		for (( k = 0; k < serverSockets; k++ ))
		{
			...
			newFile="perfStat_UDPMultiSocketReusePort"
			...
			#perf stat -e r500104,r500204,r500404,r500105,r500205,r500405,r500420,r500820,r530451,r530251,r530851,r500108,r500208,r500109,r500209,r50010a,r50020a,r50040a,r50080a,r500f0a,r50010b,r50020b,r50040b,r50080b,r50100b,r501f0b,r530426,r530126,r530326,r530526 -o $newFile -- ./server $packages 1 $(($k+1820)) &
			#perf stat -e r500104,r500204,r500404,r500105,r500205,r500405,r500420,r500820,r530451,r530251,r530851,r500108,r500208,r500109,r500209,r50010a,r50020a,r50040a,r50080a,r500f0a,r50010b,r50020b,r50040b,r50080b,r50100b,r501f0b,r530426,r530126,r530326,r530526,r5304b4,r5301b4,r5302b4,r5304b3,r5301b3,r5302b3,r5301b8,r5302b8,r5304b8,r530451,r530251,r530851,r530151,r500143,r500243,r500141,r500241,r500441,r500841,r501041,r502041,r500741,r503841,r500140,r500240,r500440,r500840,r501040,r502040,r500740,r503840,r500106,r500206,r500406,r500806,r501006,r502006,r500107,r500207,r500407,r500807,r501007,r502007,r502407,r500132,r500232,r500432,r500732 -o $newFile -- ./server $packages 1 $(($k+1820)) &
			perf stat -e r500104,r500204,r500404,r500105,r500205,r500405,r500420,r500820,r530451,r530251,r530851,r500108,r500208,r500109,r500209,r50010a,r50020a,r50040a,r50080a,r500f0a,r50010b,r50020b,r50040b,r50080b,r50100b,r501f0b,r530426,r530126,r530326,r530526,r5304b4,r5301b4,r5302b4,r5304b3,r5301b3,r5302b3,r5301b8,r5302b8,r5304b8,r530451,r530251,r530851,r530151,r500143,r500243,r500141,r500241,r500441,r500841,r501041,r502041,r500741,r503841,r500140,r500240,r500440,r500840,r501040,r502040,r500740,r503840,r500106,r500206,r500406,r500806,r501006,r502006,r500107,r500207,r500407,r500807,r501007,r502007,r502407,r500132,r500232,r500432,r500732 -o $newFile -- ./serverReuse --packets $packages --threads 1 --port 1820 --reuseport &
		}
		...
		for (( k = 0; k < serverSockets; k++ ))
		{
			...
			{
				#./client $(($maxPackages*10)) 127.0.0.1 $(($k+1820)) &
				./client $(($maxPackages*10)) 127.0.0.1 1820 &
			}
		}
		#wait $pid
		wait $(pgrep 'server')
...
python postProcessing.py $repetitions
mv SummaryResults.csv SummaryResults_UDPMultiSocketReusePort.csv
mv FullResults.csv FullResults_UDPMultiSocketReusePort.csv
#rm *.data
